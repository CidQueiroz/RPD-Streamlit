...
from desenvolvimento_pessoal import cursos, protocolo_diario, rpd
from core.database import (
    fetch_data_as_dataframe,
    fetch_data,
    get_empresa_por_nome,
    execute_command,
)
...
from gestao.gerador_pdf import criar_recibo_venda
...
def construir_query_relatorio_vendas(
    filtro_opcao, data_selecionada, id_empresa, is_staff, user_id
):
    ...
    if filtro_opcao == "Hoje":
        ...
        data_inicio, data_fim = data_selecionada, data_selecionada + pd.Timedelta(
            days=1
        )
...
def main():
            ...
            with st.form("form_cadastro"):
                ...
                if cadastrar:
                    if not all(
                        ...
                        if not empresa_data:
                            st.error(
                                f'A empresa "{nome_empresa}" não foi encontrada. Verifique o nome ou peça ao seu gerente para cadastrá-la.'
                            )
                        else:
                            id_empresa = empresa_data[0]["id_empresa"]
                            if adicionar_usuario(
                                novo_nome, novo_usuario, nova_senha, id_empresa
                            ):
                                st.success(
    ...
    if not user_details:
        st.error(
            "Não foi possível carregar os detalhes do usuário. Tente fazer login novamente."
        )
        st.stop()
    ...
    if opcao == "Estoque":
        ...
        if st.session_state["is_staff"]:
            ...
            with st.form("form_add_item", clear_on_submit=True):
                ...
                preco = st.number_input(
                    ...
                    min_value=0.01,
                    format="%.2f",
                    help="O preço só é necessário para itens novos.",
                )
        ...
        if df_estoque.empty:
            ...
            df_estoque_display["preco"] = pd.to_numeric(
                df_estoque_display["preco"]
            ).map("R$ {:,.2f}".format)
            df_estoque_display = df_estoque_display[
                ["id_item", "item", "variacao", "quantidade", "preco"]
            ]
        ...
        if (
            "last_sale_details" in st.session_state
            and st.session_state.last_sale_details
        ):
            ...
            if not df_estoque.empty:
                with st.form("form_venda"):
                    itens_disponiveis = [
                        f"{row['item']} - {row['variacao']} (ID: {row['id_item']})"
                        for index, row in df_estoque.iterrows()
                    ]
                    item_vendido_str = st.selectbox(
                        "Selecione o item vendido", itens_disponiveis
                    )
                ...
                st.warning(
                    "Nenhum item em estoque para vender. Adicione itens na página de Estoque."
                )
        ...
        try:
            ...
            filtro_opcao = st.selectbox(
                "Filtrar período:",
                ["Hoje", "Ontem", "Todo o período", "Selecionar data"],
            )
            ...
            st.metric(
                label="Total Arrecadado (filtrado)", value=f"R$ {total_arrecadado:,.2f}"
            )
            ...
            if df_filtrado.empty:
                ...
                df_display["Preço Unitário"] = pd.to_numeric(
                    df_display["Preço Unitário"]
                ).map("R$ {:,.2f}".format)
                df_display["Preço Total"] = pd.to_numeric(
                    df_display["Preço Total"]
                ).map("R$ {:,.2f}".format)
                st.table(df_display)
            ...
            if st.session_state["is_staff"]:
                ...
                data_comissao = [
                    {
                        "Nome": nome,
                        "Percentual": f"{p:.0%}",
                        "Valor": f"R$ {total_arrecadado * p:,.2f}",
                    }
                    for nome, p in comissoes.items()
                ]
                ...
                df_estoque_baixo = fetch_data_as_dataframe(
                    "SELECT item, variacao, quantidade, preco FROM estoque WHERE quantidade <= :threshold AND empresa_fk = :id_empresa",
                    {
                ...
                if df_estoque_baixo.empty:
                    st.info(
                        f"Nenhum item com estoque igual ou inferior a {LOW_STOCK_THRESHOLD} unidades."
                    )
                else:
                    df_estoque_baixo["preco"] = pd.to_numeric(
                        df_estoque_baixo["preco"]
                    ).map("R$ {:,.2f}".format)
        ...
        if submitted:
            datahora = datetime.now(pytz.timezone("America/Sao_Paulo")).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            if rpd.salvar_resposta(
                datahora,
                situacao,
                pensamentos,
                emocao,
        ...
        protocolo_diario.exibir_protocolo_diario(
            st.session_state["user_id"], st.session_state["is_staff"]
        )
