...
def ler_cursos_sql(user_id):
    ...
    try:
        ...
        if df is None:
            st.warning(
                "Ocorreu um erro ao buscar os cursos, o DataFrame retornou nulo."
            )
            return pd.DataFrame()
        ...
        if not df.empty:
            df["Percentual"] = (
                pd.to_numeric(df["Percentual"], errors="coerce").fillna(0).astype(int)
            )
            df["Fase do curso"] = (
                pd.to_numeric(df["Fase do curso"], errors="coerce")
                .fillna(1)
                .astype(int)
            )
...
def atualizar_progresso_sql(nome_curso, novo_percentual, user_id):
    ...
    try:
        ...
        if rows_affected is not None and rows_affected > 0:
            ...
            st.warning(
                f"Nenhum curso com o nome '{nome_curso}' foi encontrado para o seu usuário."
            )
...
def deletar_curso_sql(id_curso, user_id):
    ...
    try:
        query = (
            "DELETE FROM cursos WHERE id_curso = :id_curso AND usuario_fk = :user_id"
        )
...
def exibir_painel_cursos_sql(user_id):
    ...
    st.markdown(
        "O mapa estratégico da sua evolução. Monitore seu progresso e mantenha o foco na missão atual."
    )
    ...
    with st.expander("Gerenciar Cursos"):
        ...
        curso_selecionado_nome = st.selectbox(
            "Selecione um curso para editar ou adicione um novo:", cursos_list
        )

        curso_atual = None
        if curso_selecionado_nome != "Adicionar Novo Curso":
            curso_atual = df_cursos[
                df_cursos["Nome do curso"] == curso_selecionado_nome
            ].iloc[0]

        with st.form("form_curso", clear_on_submit=True):
            nome = st.text_input(
                "Nome do curso",
                value=curso_atual["Nome do curso"] if curso_atual is not None else "",
            )
            fase = st.number_input(
                ...
                step=1,
                value=(
                    int(curso_atual["Fase do curso"]) if curso_atual is not None else 1
                ),
            )
            motivacao = st.text_area(
                "Motivação",
                value=curso_atual["Motivação"] if curso_atual is not None else "",
            )
            prazo = st.text_input(
                "Prazo para conclusão",
                value=curso_atual["Prazo conclusão"] if curso_atual is not None else "",
            )
            link = st.text_input(
                "Link do curso",
                value=curso_atual["Link do curso"] if curso_atual is not None else "",
            )
            obs = st.text_area(
                "Observação",
                value=curso_atual["Observação"] if curso_atual is not None else "",
            )

            col1, col2, col3 = st.columns([2, 1, 2])
            with col1:
                submitted_save = st.form_submit_button(
                    "Salvar Curso", use_container_width=True, type="primary"
                )
            if curso_atual is not None:
                with col3:
                    submitted_delete = st.form_submit_button(
                        "Deletar", use_container_width=True, type="secondary"
                    )
