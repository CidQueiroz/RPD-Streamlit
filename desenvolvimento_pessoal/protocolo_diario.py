...
def salvar_log_pod(registros_df, usuario_id):
    ...
    try:
        ...
        execute_command("DELETE FROM log_pod_diario WHERE DATE(data) = :data AND usuario_fk = :uid", {"data": hoje_str, "uid": usuario_id})
        ...
        for _, row in registros_df.iterrows():
            ...
            execute_command(
                ...
                {"data": data_completa, "status": 1 if row['status'] else 0, "uid": usuario_id, "aid": row['atividade_id']}
            )
    except Exception as e:
...
def exibir_gerenciamento_atividades(usuario_id: int):
    ...
    with tab1:
        with st.form("adicionar_atividade"):
            ...
            periodo = st.selectbox("Período", options=list(PERIODOS.keys()), format_func=lambda x: PERIODOS[x])
            if st.form_submit_button("Adicionar Atividade") and nome_atividade.strip():
                if adicionar_nova_atividade(nome_atividade.strip(), periodo, usuario_id):
                    st.rerun()
    
    with tab2:
        atividades_ativas = carregar_atividades_por_periodo(usuario_id, apenas_ativas=True)
        todas_ativas = [ativ for periodo_atividades in atividades_ativas.values() for ativ in periodo_atividades]
        if todas_ativas:
            opcoes = [f"{ativ['nome']} (ID: {ativ['id']})" for ativ in todas_ativas]
            atividade_selecionada = st.selectbox("Selecione a atividade para desativar", opcoes, key="desativar_select")
    ...
    with tab3:
        ...
        atividades_inativas = fetch_data_as_dataframe(query_inativas, {"uid": usuario_id})
        if not atividades_inativas.empty:
            opcoes_inativas = [f"{row['nome_atividade']} (ID: {row['id_atividade']})" for _, row in atividades_inativas.iterrows()]
            atividade_para_reativar = st.selectbox("Selecione a atividade para reativar", opcoes_inativas, key="reativar_select")
...
def exibir_protocolo_diario(usuario_id: int, is_staff: bool):
        ...
        for periodo_key, nome_periodo in PERIODOS.items():
            ...
            if atividades_periodo:
                ...
                for atividade in atividades_periodo:
                    ...
                    novo_status = st.checkbox(atividade['nome'], value=status_inicial, key=f"cb_{atividade['id']}")
                    if novo_status:
                        registros_para_salvar.append({'data': hoje_str, 'atividade_id': atividade['id'], 'status': True})
    ...
    with col2:
        ...
        percentual_concluido = (atividades_concluidas_hoje / total_atividades) if total_atividades > 0 else 0.0
        fig = go.Figure(go.Bar(y=[100], marker_color='rgba(0,0,0,0)', marker_line_color='gray', marker_line_width=1.5, hoverinfo='none'))
        fig.add_trace(go.Bar(y=[percentual_concluido * 100], texttemplate='%{y:.0f}%', textposition='inside', marker_color='#0068C9', hoverinfo='none'))
        fig.update_layout(barmode='overlay', yaxis_range=[0, 100], xaxis_showticklabels=False, plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=0, b=0), height=500, showlegend=False)
    ...
    if not log_concluido_30_dias.empty:
        ...
        percentual_diario = (progresso_diario / total_atividades) * 100 if total_atividades > 0 else 0
